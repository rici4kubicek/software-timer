...
void test_Timer_Remaining_BasicFlow(void)
{
    ...
    TEST_ASSERT_EQUAL(100, SoftwareTimer_Remaining(&timer));

    // After 25 ticks, should have 75 remaining
    advance_time(25);
    TEST_ASSERT_EQUAL(75, SoftwareTimer_Remaining(&timer));

    // After 50 ticks total, should have 50 remaining
    advance_time(25);
    TEST_ASSERT_EQUAL(50, SoftwareTimer_Remaining(&timer));
...
void test_Timer_Remaining_ClockOverflow(void)
{
    ...
    TEST_ASSERT_EQUAL(25, SoftwareTimer_Remaining(&timer));
...
void test_Timer_Remaining_MaxInterval(void)
{
    ...
    SoftwareTimer_Set(&timer, UINT32_MAX);

    // Should have max interval remaining
    TEST_ASSERT_EQUAL(UINT32_MAX, SoftwareTimer_Remaining(&timer));

    // Should not expire even after large time advance
    advance_time(UINT32_MAX - 1);
    TEST_ASSERT_EQUAL(1, SoftwareTimer_Remaining(&timer));
...
void test_Timer_Remaining_ConsistencyWithIsExpired(void)
{
    ...
    SoftwareTimer_Set(&timer, 1000);

    // Test at various points
    for (uint32_t i = 0; i < 1000; i += 100) {
        advance_time(100);

        bool expired = SoftwareTimer_IsExpired(&timer);
        uint32_t remaining = SoftwareTimer_Remaining(&timer);
...
void test_Timer_MultipleTimers(void)
{
    ...
    SoftwareTimer_Set(&timer1, 50);
    SoftwareTimer_Set(&timer2, 100);
    SoftwareTimer_Set(&timer3, 150);

    // No timers should expire initially
    TEST_ASSERT_FALSE(SoftwareTimer_IsExpired(&timer1));
    TEST_ASSERT_FALSE(SoftwareTimer_IsExpired(&timer2));
    TEST_ASSERT_FALSE(SoftwareTimer_IsExpired(&timer3));

    // First timer expires
    advance_time(50);
    TEST_ASSERT_TRUE(SoftwareTimer_IsExpired(&timer1));
    TEST_ASSERT_FALSE(SoftwareTimer_IsExpired(&timer2));
    TEST_ASSERT_FALSE(SoftwareTimer_IsExpired(&timer3));
    ...
    advance_time(50);
    TEST_ASSERT_TRUE(SoftwareTimer_IsExpired(&timer1));
    TEST_ASSERT_TRUE(SoftwareTimer_IsExpired(&timer2));
    TEST_ASSERT_FALSE(SoftwareTimer_IsExpired(&timer3));
...
void test_Timer_LargeInterval(void)
{
    ...
    SoftwareTimer_Set(&timer, 0x80000000UL); // Large interval
